...
static int (*(solver_fns[]))(solver_state *) = {&trivial_deductions, &dline_deductions, &linedsf_deductions,
                                                &loop_deductions, };
...
static grid_type grid_types[] = {GRID_SQUARE, GRID_TRIANGULAR, GRID_HONEYCOMB, GRID_SNUBSQUARE, GRID_CAIRO,
                                 GRID_GREATHEXAGONAL, GRID_OCTAGONAL, GRID_KITE, GRID_FLORET, GRID_DODECAGONAL,
                                 GRID_GREATDODECAGONAL, GRID_PENROSE_P2, GRID_PENROSE_P3, };
...
static int trivial_deductions(solver_state *sstate)
{
    ...
    for (i = 0; i < g->num_dots; i++) {
        grid_dot *d = g->dots + i;
        int yes, no, unknown; // 分别代表确定有，确定无，不确定有无的数量
